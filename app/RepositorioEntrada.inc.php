...
class RepositorioEntrada {
    ...
    public static function obtener_todas_fecha_desc($conexion) {
        $entradas = [];
    ...
    public static function obtener_todas_completas_fecha_desc($conexion) {
        $entradas = [];
    ...
    public static function obtener_entrada_url($conexion, $url) {
        ...
        if (isset($conexion)) {
            try {
                ...
                if (!empty($resultado)) {
                    $entrada = new Entrada($resultado['id'],
                                           $resultado['autor_id'],
                                           $resultado['url'],
                                           $resultado['titulo'],
                                           $resultado['texto'],
    ...
    public static function obtener_entradas_usuario_fecha_desc($conexion, $id_usuario) {
        ...
        if (isset($conexion)) {
            try {
                ...
                $sql .= " FROM entradas a ";
                $sql .= " LEFT JOIN comentarios b ON a.id = b.entradas_id ";
                $sql .= " WHERE a.autor_id = :autor_id ";
                $sql .= " GROUP BY a.id ORDER BY a.fecha DESC";
    ...
    public static function eliminar_comentarios_entrada($conexion, $entradas_id) {
        if(isset($conexion)) {
            try {
                $conexion -> beginTransaction();

                $sql1  = "DELETE FROM comentarios WHERE entradas_id = :entradas_id";

                $sentencia1 = $conexion -> prepare($sql1);

                $sentencia1 -> bindParam(':entradas_id', $entradas_id, PDO::PARAM_STR);

                $sentencia1 -> execute();

                $sql2  = "DELETE FROM entradas WHERE id = :entradas_id";

                $sentencia2 = $conexion -> prepare($sql2);

                $sentencia2 -> bindParam(':entradas_id', $entradas_id, PDO::PARAM_STR);
    ...
    public static function buscar_entradas_por_autor($conexion, $termino_busqueda, $orden) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "SELECT e.* FROM entradas e JOIN usuarios u ON u.id = e.autor_id WHERE u.nombre LIKE :autor ORDER BY e.fecha $orden LIMIT 25";
